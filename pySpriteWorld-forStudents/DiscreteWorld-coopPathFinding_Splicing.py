    ...
    for i in range(iterations):
        ...
        for j in range(nbPlayers): # on fait bouger chaque joueur séquentiellement
            if not found[j]:
                ...
                t1 = time.clock()
                temps += t1 - t0
                if ((next_row,next_col) not in wallStates) and next_row>=0 and next_row<nbLignes and next_col>=0 and next_col<nbColonnes:
                ...
                if (row,col) in goalStates and goalStates.index((row, col)) == j:
                    ...
                    break
    print ("scores:", score)
    print("temps " + str(temps) + " iterations" + str(iteration))
    pygame.quit()
